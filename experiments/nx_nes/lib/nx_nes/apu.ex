defmodule NxNes.APU do
  @moduledoc "Resident numeric 2A03 + MMC5 APU event step, including channel clocks, sequencers, mixer and filters. DMC/5B rejected."
  import Nx.Defn
  ...
  def pack(s) do
    if s.dmc != nil or s.sunsoft5b != nil,
      do: raise(ArgumentError, "DMC and Sunsoft 5B are not implemented in this probe")

    s |> Map.from_struct() |> Map.drop([:samples, :dmc, :sunsoft5b]) |> pack_map()
  end
  ...
  defn mix(s) do
    ...
    Nx.take(pulse_table, p) + Nx.take(tnd_table, 3 * tri + 2 * noise) + expansion
  end
