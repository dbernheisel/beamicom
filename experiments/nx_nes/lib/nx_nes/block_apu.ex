defmodule NxNes.BlockAPU do
  ...
  def events(entries, cycles, capacity \\ 128) do
    ...
    Enum.reduce(entries, 0, fn {t, a, v}, last ->
      supported = a in 0x4000..0x400F or a in [0x4015, 0x4017] or a in 0x5000..0x5015
      ...
      if a == 0x4015 and v >= 0 and Bitwise.band(v, 16) != 0,
        do: raise(ArgumentError, "DMC is not supported")
    ...
    {s, pos, index, pcm, count, _, _, _} =
      while {s, pos = Nx.tensor(0, type: :s32), index = Nx.tensor(0, type: :s32),
        ...
        if next_write == pos do
          ...
          {s, used, pcm, count} = segment(s, dc, pcm, count)
          {s, pos + used, index, pcm, count, events, event_count, cycles}
        end
      end
  ...
  defnp segment(s, dc, pcm, count) do
    ...
    values = APU.mix(clock(s, offsets))
    final = clock(s, used)
