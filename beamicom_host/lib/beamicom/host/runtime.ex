defmodule Beamicom.Host.Runtime do
  @moduledoc """
  ...
  def load(system, media, options \\ []), do: Registry.load(system, media, options)
  ...
  def init(options) do
    ...
    case machine(options, system) do
      {:ok, machine} ->
        state = %__MODULE__{
          system: system,
          machine: machine,
  ...
  defp machine(options, system) do
    case Keyword.fetch(options, :machine) do
      {:ok, machine} ->
        {:ok, machine}

      :error ->
        load(system, Keyword.fetch!(options, :media), Keyword.get(options, :load_options, []))
    end
  end
