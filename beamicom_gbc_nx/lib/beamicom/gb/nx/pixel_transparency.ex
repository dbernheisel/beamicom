defmodule Beamicom.GB.Nx.PixelTransparency do
  ...
  @spec filter(binary(), {pos_integer(), pos_integer()}, {pos_integer(), pos_integer()}, keyword()) ::
          binary()
  ...
  def filter_tensor(
    ...
    key =
      {__MODULE__, @compiled_version, Nx.type(source), source_width, source_height, output_width,
       output_height}
  ...
  defn render(source, output_x, output_y, parameters) do
    ...
    polarizer = mix(Nx.broadcast(1.0, {1, 1, 3}), polarizer, parameter(parameters, :polarizer_tint))
  ...
  defnp apply_shadow(background, source, u, v, parameters, output_width, output_height) do
    ...
    deadzone = (1.0 - parameter(parameters, :threshold)) * parameter(parameters, :shadow_opacity)
