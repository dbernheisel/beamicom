defmodule Beamicom.NES.APURenderer do
  ...
  operations, elapsed CPU cycles, and the native DMC DAC plus mapper expansion
  level at every output sample. Implementations may use ordinary Elixir or an
  optional tensor backend.
  """

  @type event :: {non_neg_integer(), non_neg_integer(), integer()}

  @callback prepare(struct()) :: term()
  @callback render(term(), [event()], non_neg_integer(), [{0..127, float()}]) ::
